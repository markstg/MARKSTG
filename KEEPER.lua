...
file:close()
sleep(1)
os.execute(' screen -S MARKSTG ./MARKSTG')
end
...
MARKSTG_SUDO = sudos.KpOwner   -------bot owner
MARKSTG_TOKEN = sudos.Token -----MARKSTG_TOKEN
bot_id = sudos.Bot_ID ---id bot
MARKSTG = tonumber(_redis.Bot_ID)

function Run()
print('\27[93m>Developer:\27[39m'..' '..'@MARKSTG_ch')
end
------------------------boT ID   BY MARKSTG ----------------------
print(string.sub(_redis.Bot_ID,1,0))
local bot_id = redis:get(MARKSTG.."Bot:KpBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(MARKSTG.."Bot:KpSudos", user)
end
redis:setex(MARKSTG.."bot:reload", 7230, true)
redis:set(MARKSTG.."Bot:KpOwnerBot", config.KpOwner or 0)
redis:set(MARKSTG.."Bot:Run", config.Run or 0)
local Api = config.Token:match("(%d+)")
local RD = RNM or 0
if Api then
redis:set(MARKSTG.."Bot:Api_ID", Api)
end
function AuthoritiesEn()
local hash = "Bot:KpSudos"
local list = redis:smembers(MARKSTG..hash)
local OwnerKP_ = redis:get(MARKSTG.."Bot:KpOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(MARKSTG.."user:Name" .. OwnerKP_)
...
local user_info = redis:get(MARKSTG.."user:Name" .. v)
if user_info then
...
local list2 = redis:smembers(MARKSTG..hash2)
if #list2 ~= 0 then
text = text .. [[
...
redis:set(MARKSTG.."AuthoritiesEn", text)
end
...
redis:set(MARKSTG.."AuthoritiesFa", text)
end
AuthoritiesEn()
AuthoritiesFa()
end
...
redis:del(MARKSTG.."Bot:KpSudos")
redis:del(MARKSTG.."Bot:KpOwnerBot")
redis:del(MARKSTG.."Bot:Token")
redis:del(MARKSTG.."Bot:Channel")
setdata()
end
...
usr = string.gsub(usr, "%s+$", "")
usr = string.gsub(usr, "[\n\r]+", " ")
redis:set(MARKSTG.."Bot:ServerUser", usr)----------ServerUser
redis:del(MARKSTG.."MatchApi")
redis:del(MARKSTG.."Set_Our_ID")
redis:del(MARKSTG.."Open:Chats")
local KPdata = redis:get(MARKSTG.."Botid" .. bot_id) or "\n"
local BotKPdata = redis:get(MARKSTG.."KpOwnerBot" .. config.KpOwner) or "\n"
if redis:get(MARKSTG.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. KPdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. BotKPdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(MARKSTG.."KpSudos" .. user)
if SudoData then
...
redis:set(MARKSTG.."Our_ID", result.id_)
end
...
local lists = redis:smembers(MARKSTG..hashs)
redis:del(MARKSTG.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(MARKSTG.."SudoNumberGp" .. user_id)
end
end
local setnumbergp_three = function(user_id)
local hashss = "sudo:data:" .. user_id
local lists = redis:smembers(MARKSTG..hashss)
redis:del(MARKSTG.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(MARKSTG.."SudoNumberGp" .. user_id)
end
end
local list = redis:smembers(MARKSTG.."Bot:Admins")
for k, v in pairs(list) do
setnumbergp_two(v)
end
local lists = redis:smembers(MARKSTG.."Bot:KpSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(MARKSTG.."bot:reload", 7230, true)
...
local Bot_Channel = redis:get(MARKSTG.."Bot:Channel") or tostring(_redis.Channel)
local sudo_users = _config.Sudo_Users
local Kp_Owner = redis:get(MARKSTG.."Bot:KpOwnerBot")
local run = redis:get(MARKSTG.."Bot:Run") or "True"
if not redis:get(MARKSTG.."setnumbergp") then
setnumbergp()
redis:setex(MARKSTG.."setnumbergp", 5 * hour, true)
end
...
CH SOURCE : @MARKSTG_ch
]].."\n\027[00m")
-----------------------function is_KP----------by MARKSTG------------------------
...
local var = false
if redis:sismember(MARKSTG.."Bot:KpSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Kp_Owner) then
var = true
...
local var = false
if redis:sismember(MARKSTG.."Bot:KpSudos", user_id) then
var = true
end
if user_id == tonumber(Kp_Owner) then
var = true
...
local admin = redis:sismember(MARKSTG..hashsb, user_id)
if admin then
var = true
end
if redis:sismember(MARKSTG.."Bot:KpSudos", user_id) then
...
local monshi = redis:sismember(MARKSTG..hashssk, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(MARKSTG..hashs, user_id)
if monshi then
var = true
end
if admin then
...
local owner = redis:sismember(MARKSTG..hash, user_id)
if monshi then
var = true
end
if admin then
...
local momod = redis:sismember(MARKSTG..hash, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(MARKSTG..hashs, user_id)
local hashssk = "bot:monshis:" .. chat_id
local monshi = redis:sismember(MARKSTG..hashssk, user_id)
local hashss = "bot:owners:" .. chat_id
local owner = redis:sismember(MARKSTG..hashss, user_id)
local our_id = redis:get(MARKSTG.."Our_ID") or 0
if momod then
var = true
end
if owner then
...
local vipmem = redis:sismember(MARKSTG..hashsss, user_id)
if vipmem then
var = true
end
if momod then
...
local vipmems = redis:sismember(MARKSTG..hashsss, user_id)
if vipmems then
var = true
end
if redis:sismember(MARKSTG.."Bot:KpSudos", user_id) then
...
local banned = redis:sismember(MARKSTG..hash, user_id)
if banned then
var = true
end
return var
...
local var = false
local hash = "bot:gban:"
local gbanned = redis:sismember(MARKSTG..hash, user_id)
if gbanned then
var = true
end
return var
...
local muted = redis:sismember(MARKSTG..hash, user_id)
local muted2 = redis:get(MARKSTG..hash2)
if muted then
var = true
end
if muted2 then
...
if redis:get(MARKSTG.."GetTime") then
local data = redis:get(MARKSTG.."GetTime")
...
redis:setex(MARKSTG.."GetTime", 10, url)
local A = jdat.FAtime
local B = jdat.FAdate
if A and B then
local T = {time = A, date = B}
...
local var = true
if redis:get(MARKSTG.."Kpjoin1") then
local channel = ''..redis:get(MARKSTG..'Kpch1')..''
local url , res = https.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
...
redis:set(MARKSTG..'group:name'..GroupID,data.title_) end,nil) return redis:get(MARKSTG..'group:name'..GroupID) end
...
link = redis:get(MARKSTG.."bot:group:link" .. chat)
if link then
...
redis:del(MARKSTG.."bot:muteall" .. chat_id)
redis:del(MARKSTG.."bot:text:mute" .. chat_id)
redis:del(MARKSTG.."bot:photo:mute" .. chat_id)
redis:del(MARKSTG.."bot:video:mute" .. chat_id)
redis:del(MARKSTG.."bot:selfvideo:mute" .. chat_id)
redis:del(MARKSTG.."bot:gifs:mute" .. chat_id)
redis:del(MARKSTG.."anti-flood:" .. chat_id)
redis:del(MARKSTG.."flood:max:" .. chat_id)
redis:del(MARKSTG.."bot:sens:spam" .. chat_id)
redis:del(MARKSTG.."floodstatus" .. chat_id)
redis:del(MARKSTG.."bot:music:mute" .. chat_id)
redis:del(MARKSTG.."bot:bots:mute" .. chat_id)
redis:del(MARKSTG.."bot:duplipost:mute" .. chat_id)
redis:del(MARKSTG.."bot:inline:mute" .. chat_id)
redis:del(MARKSTG.."bot:cmds" .. chat_id)
redis:del(MARKSTG.."bot:bots:mute" .. chat_id)
redis:del(MARKSTG.."bot:voice:mute" .. chat_id)
redis:del(MARKSTG.."editmsg" .. chat_id)
redis:del(MARKSTG.."bot:links:mute" .. chat_id)
redis:del(MARKSTG.."bot:pin:mute" .. chat_id)
redis:del(MARKSTG.."bot:sticker:mute" .. chat_id)
redis:del(MARKSTG.."bot:tgservice:mute" .. chat_id)
redis:del(MARKSTG.."bot:webpage:mute" .. chat_id)
redis:del(MARKSTG.."bot:strict" .. chat_id)
redis:del(MARKSTG.."bot:hashtag:mute" .. chat_id)
redis:del(MARKSTG.."tags:lock" .. chat_id)
redis:del(MARKSTG.."bot:location:mute" .. chat_id)
redis:del(MARKSTG.."bot:contact:mute" .. chat_id)
redis:del(MARKSTG.."bot:english:mute" .. chat_id)
redis:del(MARKSTG.."bot:arabic:mute" .. chat_id)
redis:del(MARKSTG.."bot:forward:mute" .. chat_id)
redis:del(MARKSTG.."bot:member:lock" .. chat_id)
redis:del(MARKSTG.."bot:document:mute" .. chat_id)
redis:del(MARKSTG.."markdown:lock" .. chat_id)
redis:del(MARKSTG.."Game:lock" .. chat_id)
redis:del(MARKSTG.."bot:spam:mute" .. chat_id)
redis:del(MARKSTG.."post:lock" .. chat_id)
redis:del(MARKSTG.."bot:welcome" .. chat_id)
redis:del(MARKSTG.."delidstatus" .. chat_id)
redis:del(MARKSTG.."delpro:" .. chat_id)
redis:del(MARKSTG.."sharecont" .. chat_id)
redis:del(MARKSTG.."sayedit" .. chat_id)
redis:del(MARKSTG.."welcome:" .. chat_id)
redis:del(MARKSTG.."bot:group:link" .. chat_id)
redis:del(MARKSTG.."bot:filters:" .. chat_id)
redis:del(MARKSTG.."bot:muteall:Time" .. chat_id)
redis:del(MARKSTG.."bot:muteall:start" .. chat_id)
redis:del(MARKSTG.."bot:muteall:stop" .. chat_id)
redis:del(MARKSTG.."bot:muteall:start_Unix" .. chat_id)
redis:del(MARKSTG.."bot:muteall:stop_Unix" .. chat_id)
redis:del(MARKSTG.."bot:muteall:Run" .. chat_id)
redis:del(MARKSTG.."bot:muted:" .. chat_id)
end
...
redis:set(MARKSTG.."bot:webpage:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:inline:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:bots:mute" .. chat_id, true)
redis:set(MARKSTG.."tags:lock" .. chat_id, true)
redis:set(MARKSTG.."markdown:lock" .. chat_id, true)
redis:set(MARKSTG.."bot:links:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:hashtag:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:spam:mute" .. chat_id, true)
redis:set(MARKSTG.."anti-flood:" .. chat_id, true)
redis:set(MARKSTG.."Game:lock" .. chat_id, true)
redis:set(MARKSTG.."bot:panel" .. chat_id, "one")
end
...
redis:set(MARKSTG.."anti-flood:" .. chat_id, true)
redis:set(MARKSTG.."Game:lock" .. chat_id, true)
redis:set(MARKSTG.."post:lock" .. chat_id, true)
redis:set(MARKSTG.."bot:forward:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:photo:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:video:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:selfvideo:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:gifs:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:sticker:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:location:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:document:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:panel" .. chat_id, "two")
end
...
redis:set(MARKSTG.."bot:links:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:spam:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:sens:spam" .. chat_id, 500)
redis:set(MARKSTG.."anti-flood:" .. chat_id, true)
redis:set(MARKSTG.."Game:lock" .. chat_id, true)
redis:set(MARKSTG.."bot:cmds" .. chat_id, true)
redis:set(MARKSTG.."bot:duplipost:mute" .. chat_id, true)
redis:set(MARKSTG.."bot:panel" .. chat_id, "three")
end
...
local user_id
local user = false
local list1 = redis:smembers(MARKSTG.."Bot:KpSudos")
local list2 = redis:smembers(MARKSTG.."Bot:Admins")
for k, v in pairs(list1) do
local hash = "sudo:data:" .. v
local is_add = redis:sismember(MARKSTG..hash, chat)
if is_add then
user_id = v
end
end
...
if redis:sismember(MARKSTG..hash, chat) then
user_id = Kp_Owner
end
if user_id then
local user_info = redis:get(MARKSTG.."user:Name" .. user_id)
if user_info then
user = user_info
end
end
...
if hash then
local names = redis:hkeys(MARKSTG..hash)
local text = ""
for i = 1, #names do
...
redis:set(MARKSTG.."bot:Uptime", os.time())
----------------tdcli_update_callback---------------------------------------------------------------------------
function tdcli_update_callback(data)
local our_id = redis:get(MARKSTG.."Our_ID") or 0
local api_id = redis:get(MARKSTG.."Bot:Api_ID") or 0
if data.ID == "UpdateNewMessage" then
local msg = data.message_
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
redis:sadd(MARKSTG.."groups:users" .. msg.chat_id_, msg.sender_user_id_)--save users gp
redis:incr(MARKSTG.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_.."")--save msgs gp
...
✓‏
‌‏]]
send(sudoed, 0, 1,iD_MARKSTG, 1, "md")
redis:del(MARKSTG.."bot:enable:" .. msg.chat_id_)
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
end end end
if msg.content_.ID == "MessageChatAddMembers" then
redis:incr(MARKSTG..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.date_ < os.time() - 40 then
print("\027[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> OLD MSG <<<\027[00m")
return false
end
if not redis:get(MARKSTG.."Set_Our_ID") then
...
if not redis:get(MARKSTG.."Rank:Data") then
...
redis:set(MARKSTG.."KpSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(MARKSTG.."KpOwnerBot" .. Kp_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
redis:set(MARKSTG.."Botid" .. result.id_, "> Bot ID : " .. botuserid .. [[
...
redis:set(MARKSTG.."Botid" .. bot_id, [[
> Bot ID : ---
> Bot Name : ---
> Bot Username : ---
---------------]])
...
redis:setex(MARKSTG.."Rank:Data", 700, true)
end
if redis:get(MARKSTG.."bot:reload") and 30 > tonumber(redis:ttl(MARKSTG.."bot:reload")) then
load_config()
setnumbergp()
redis:setex(MARKSTG.."bot:reload", 7230, true)
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m>>> Bot Reloaded <<<\027[00m")
end
if not redis:get(MARKSTG.."bot:reload2") then
redis:del(MARKSTG.."bot:groups")
redis:del(MARKSTG.."bot:userss")
redis:setex(MARKSTG.."bot:reloadingtime", 22 * hour, true)
redis:setex(MARKSTG.."bot:reload2", week, true)
redis:setex(MARKSTG.."bot:reload3", 2222 * day, true)
redis:setex(MARKSTG.."bot:reload4", 2222 * day, true)
end
if redis:get(MARKSTG.."bot:reload3") and 500 >= tonumber(redis:ttl(MARKSTG.."bot:reload3")) then
local hash = "bot:groups"
local list = redis:smembers(MARKSTG..hash)
for k, v in pairs(list) do
if not redis:get(MARKSTG.."bot:enable:" .. v) and not redis:get(MARKSTG.."bot:charge:" .. v) then
resetgroup(v)
chat_leave(v, bot_id)
redis:srem(MARKSTG..hash, v)
end
end
redis:del(MARKSTG.."bot:reload3")
end
if redis:get(MARKSTG.."bot:reload4") and redis:ttl(MARKSTG.."bot:reload4") <= 600 then
local reload_data_sudo = function()
local hashsudo = "Bot:KpSudos"
local listsudo = redis:smembers(MARKSTG..hashsudo)
for k, v in pairs(listsudo) do
local hashdata = "sudo:data:" .. v
local listdata = redis:smembers(MARKSTG..hashdata)
for k, gp in pairs(listdata) do
if not redis:sismember(MARKSTG.."bot:groups", gp) then
redis:srem(MARKSTG..hashdata, gp)
end
end
end
end
...
local listadmin = redis:smembers(MARKSTG..hashadmin)
...
------------------------------EXpirepannel GP ----------------------------------------------------------
local expiretime = redis:ttl(MARKSTG.."bot:charge:" .. msg.chat_id_)
if not redis:get(MARKSTG.."bot:charge:" .. msg.chat_id_) and redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) then
redis:del(MARKSTG.."bot:enable:" .. msg.chat_id_)
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
end
if redis:get(MARKSTG.."bot:charge:" .. msg.chat_id_) and not redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) then
redis:set(MARKSTG.."bot:enable:" .. msg.chat_id_, true)
end
if not redis:get(MARKSTG.."bot:expirepannel:" .. msg.chat_id_) and redis:get(MARKSTG.."bot:charge:" .. msg.chat_id_) and tonumber(expiretime) < tonumber(day) and tonumber(expiretime) >= 3600 then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
local v = tonumber(Kp_Owner)
local list = redis:smembers(MARKSTG.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(MARKSTG.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
owner = user_info
else
...
send(v, 0, 1, "💲┊ سوف تنتهي صلاحيه المجموعه\n🔅┊الرابط : " .. (redis:get(MARKSTG.."bot:group:link" .. msg.chat_id_) or "لا يوجد ") .. "\n🚫┊ الايدي » " .. msg.chat_id_ .. "", 1, "html")
redis:setex(MARKSTG.."bot:expirepannel:" .. msg.chat_id_, 43200, true)
end
end
------------------------Autoleave FOR BOT----------------------------------------------------
if redis:get(MARKSTG.."autoleave") == "On" then
local id = tostring(msg.chat_id_)
if not redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) and id:match("-100(%d+)") and not redis:get(MARKSTG.."bot:autoleave:" .. msg.chat_id_) then
redis:setex(MARKSTG.."bot:autoleave:" .. msg.chat_id_, 1400, true)
end
local autoleavetime = tonumber(redis:ttl(MARKSTG.."bot:autoleave:" .. msg.chat_id_))
local time = 400
if tonumber(autoleavetime) < tonumber(time) and tonumber(autoleavetime) > 150 then
redis:set(MARKSTG.."lefting" .. msg.chat_id_, true)
end
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") and redis:get(MARKSTG.."lefting" .. msg.chat_id_) then
if not redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) and not redis:get(MARKSTG.."bot:charge:" .. msg.chat_id_) then
redis:del(MARKSTG.."lefting" .. msg.chat_id_)
redis:del(MARKSTG.."bot:autoleave:" .. msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعه\n🔱┊ الاسم » 👇🏾\n🏮┊ ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
elseif redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) then
redis:del(MARKSTG.."lefting" .. msg.chat_id_)
end
end
elseif redis:get(MARKSTG.."bot:charge:" .. msg.chat_id_) == "Trial" and 500 > redis:ttl(MARKSTG.."bot:charge:" .. msg.chat_id_) then
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعه\n🔱┊ الاسم » ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
redis:del(MARKSTG.."bot:charge:" .. msg.chat_id_)
end
...
redis:set(MARKSTG..svgroup, chatname)
end
end
...
local id = result.id_
if username then
redis:set(MARKSTG..svuser, "@" .. username)
else
redis:set(MARKSTG..svuser, name)
end
end
getUser(msg.sender_user_id_, check_username)
if redis:get(MARKSTG.."clerk") == "On" then
local clerk = function(extra, result)
if not is_admin(result.id_) then
local textc = redis:get(MARKSTG.."textsec")
if not redis:get(MARKSTG.."secretary_:" .. msg.chat_id_) and textc then
...
redis:setex(MARKSTG.."secretary_:" .. msg.chat_id_, day, true)
end
end
end
...
local status_welcome = (redis:get(MARKSTG..'status:welcome:'..msg.chat_id_) or 'disable')
...
if redis:get(MARKSTG..'welcome:'..msg.chat_id_) then
text = redis:get(MARKSTG..'welcome:'..msg.chat_id_)
else
...
----------------- save all msg bot --------------------------
redis:incr(MARKSTG.."bot:allmsgs")
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
if not redis:sismember(MARKSTG.."bot:groups", msg.chat_id_) then
redis:sadd(MARKSTG.."bot:groups", msg.chat_id_)
end
elseif id:match("^(%d+)") then
if not redis:sismember(MARKSTG.."bot:userss", msg.chat_id_) then
redis:sadd(MARKSTG.."bot:userss", msg.chat_id_)
end
elseif not redis:sismember(MARKSTG.."bot:groups", msg.chat_id_) then
redis:sadd(MARKSTG.."bot:groups", msg.chat_id_)
end
end
---------------type the msg--------------------------------------------------
...
redis:incr(MARKSTG.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")
text = msg.content_.text_
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Text ] <<\027[00m")
msg_type = "MSG:Text"
end
if msg.content_.ID == "MessagePhoto" then
redis:incr(MARKSTG.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(MARKSTG.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")
if not redis:get(MARKSTG.."lock_STCK"..msg.chat_id_) then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"منور انت ، 😙","اه باع وجهك شكد نضيف 😅","هذا منو ، 😏","تسمحلي ابوسك ☹️😹","مليان ضحك مليان 😹❤️","تف على هذا  ويهك 💦😹","اذا حاته ممكن الرقم 😆😹","تدري صار "..(redis:get(MARKSTG.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")).." ملصق داز  شهالتبذير 🤔😹","كافي ملصقات مشايف 😫"}
send(msg.chat_id_, msg.id_, 1,""..MARKSTG[math.random(#MARKSTG)].."", 1, 'md')
end
end
...
redis:incr(MARKSTG.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Voice ] <<\027[00m")
msg_type = "MSG:Voice"
end
if msg.content_.ID == "MessageVideo" then
redis:incr(MARKSTG.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Video ] <<\027[00m")
msg_type = "MSG:Video"
end
if msg.content_.ID == "MessageAnimation" then
redis:incr(MARKSTG.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(MARKSTG.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
local content_text = redis:get(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_text == 'save_repgp' then
redis:del(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
local content_text = redis:get(MARKSTG..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then
redis:set(MARKSTG..'video_repgp'..content_text..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(MARKSTG..'stecker_repgp'..content_text..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(MARKSTG..'voice_repgp'..content_text..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(MARKSTG..'gif_repgp'..content_text..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(MARKSTG..'text_repgp'..content_text..''..msg.chat_id_..'', msg.content_.text_)
end
redis:sadd('rep_owner'..msg.chat_id_..'',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(MARKSTG..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end end
if msg.content_.text_ and not redis:get(MARKSTG..'lock_reeeep'..msg.chat_id_) then
if redis:get(MARKSTG..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(MARKSTG..'video_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(MARKSTG..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(MARKSTG..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if  redis:get(MARKSTG..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(MARKSTG..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(MARKSTG..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(MARKSTG..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(MARKSTG..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
send(msg.chat_id_, msg.id_, 1, redis:get(MARKSTG..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') ,  1, 'md')
end
end
---------------------------------in all gps---------------------------------------------------
text = msg.content_.text_
if msg.content_.text_  or msg.content_.video_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ then
local content_text = redis:get(MARKSTG.."add:repallt"..msg.sender_user_id_)
if content_text == 'save_rep' then
redis:del(MARKSTG.."add:repallt"..msg.sender_user_id_)
local content_text = redis:get(MARKSTG.."addreply2:"..msg.sender_user_id_)
if msg.content_.video_ then
redis:set(MARKSTG.."video_repall"..content_text, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(MARKSTG.."stecker_repall"..content_text, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(MARKSTG.."voice_repall"..content_text, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(MARKSTG.."gif_repall"..content_text, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(MARKSTG.."text_repall"..content_text, msg.content_.text_)
end
redis:sadd('rep_sudo',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(MARKSTG.."addreply2:"..msg.sender_user_id_)
return false end end
if msg.content_.text_ and not redis:get(MARKSTG..'lock_reeeep'..msg.chat_id_) then
if redis:get(MARKSTG.."video_repall"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(MARKSTG.."video_repall"..msg.content_.text_))
end
if redis:get(MARKSTG.."voice_repall"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(MARKSTG.."voice_repall"..msg.content_.text_))
end
if  redis:get(MARKSTG.."gif_repall"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(MARKSTG.."gif_repall"..msg.content_.text_))
end
if redis:get(MARKSTG.."stecker_repall"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(MARKSTG.."stecker_repall"..msg.content_.text_))
end
if redis:get(MARKSTG.."text_repall"..msg.content_.text_) then
send(msg.chat_id_, msg.id_, 1, redis:get(MARKSTG.."text_repall"..msg.content_.text_) ,  1, "md")
end
end
...
if not redis:get(MARKSTG..flmax) then
floodMax = 5
else
floodMax = tonumber(redis:get(MARKSTG..flmax))
end
local pm = "flood:" .. msg.sender_user_id_ .. ":" .. msg.chat_id_ .. ":msgs"
if not redis:get(MARKSTG..pm) then
msgs = 0
else
msgs = tonumber(redis:get(MARKSTG..pm))
end
local TIME_CHECK = 2
local TIME_CHECK_PV = 2
local hashflood = "anti-flood:" .. msg.chat_id_
if msgs > floodMax - 1 then
if redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) == "Kicked" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
elseif redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) == "DelMsg" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
else
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
end
...
if not redis:get(MARKSTG..pmonpv) then
msgsonpv = 0
else
msgsonpv = tonumber(redis:get(MARKSTG..pmonpv))
end
if msgsonpv > 12 then
blockUser(msg.sender_user_id_)
end
local idmem = tostring(msg.chat_id_)
if idmem:match("^(%d+)") and not is_admin(msg.sender_user_id_) and not redis:get(MARKSTG.."Filtering:" .. msg.sender_user_id_) then
redis:setex(MARKSTG..pmonpv, TIME_CHECK_PV, msgsonpv + 1)
end
...
if redis:get(MARKSTG.."keed_link"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
local chat = msg.chat_id_
delete_msg(chat, msgs)
HTTPS.request("https://api.telegram.org/bot" .. MARKSTG_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(MARKSTG..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(MARKSTG.."keed_fosh"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_user"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(MARKSTG.."keed_photo" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_text"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if text and text:match("(.*)(#)(.*)")  then
if redis:get(MARKSTG.."keed_hashtag"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_fwd"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageSticker" then
if redis:get(MARKSTG.."keed_stecker"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAudio" then
if redis:get(MARKSTG.."keed_audio"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVoice" then
if redis:get(MARKSTG.."keed_voice"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVideo" then
if redis:get(MARKSTG.."keed_video"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAnimation" then
if redis:get(MARKSTG.."keed_gif"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageContact" then
if redis:get(MARKSTG.."keed_contect"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_arbic"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageDocument" then
if redis:get(MARKSTG.."keed_Document"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_english"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."keed_markdon"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(MARKSTG.."bot:muted:Time" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and redis:sismember(MARKSTG.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_) then
redis:srem(MARKSTG.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_)
end
if is_gbanned(msg.sender_user_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
return
end
if redis:get(MARKSTG.."bot:muteall:Time" .. msg.chat_id_) then
local start_ = redis:get(MARKSTG.."bot:muteall:start" .. msg.chat_id_)
local start = start_:gsub(":", "")
local stop_ = redis:get(MARKSTG.."bot:muteall:stop" .. msg.chat_id_)
local stop = stop_:gsub(":", "")
local SVTime = os.date("%R")
local Time = SVTime:gsub(":", "")
if tonumber(Time) >= tonumber(start) and not redis:get(MARKSTG.."bot:muteall:Run" .. msg.chat_id_) then
local g = os.time()
redis:set(MARKSTG.."bot:muteall:start_Unix" .. msg.chat_id_, g)
...
redis:set(MARKSTG.."bot:muteall:stop_Unix" .. msg.chat_id_, unix)
redis:set(MARKSTG.."bot:muteall:Run" .. msg.chat_id_, true)
end
end
if redis:get(MARKSTG.."bot:muteall:Time" .. msg.chat_id_) and redis:get(MARKSTG.."bot:muteall:Run" .. msg.chat_id_) then
local SR = redis:get(MARKSTG.."bot:muteall:start_Unix" .. msg.chat_id_) or 0
local SP = redis:get(MARKSTG.."bot:muteall:stop_Unix" .. msg.chat_id_) or 0
...
redis:del(MARKSTG.."bot:muteall:Run" .. msg.chat_id_)
end
end
---------------------pinnedmsg----------------------------------------------------------
if msg.content_.ID == "MessagePinMessage" and not msg.sender_user_id_ == our_id and not is_owner(msg.sender_user_id_, msg.chat_id_) and redis:get(MARKSTG.."pinnedmsg" .. msg.chat_id_) and redis:get(MARKSTG.."bot:pin:mute" .. msg.chat_id_) then
unpinmsg(msg.chat_id_)
local pin_id = redis:get(MARKSTG.."pinnedmsg" .. msg.chat_id_)
pinmsg(msg.chat_id_, pin_id, 0)
end
if not redis:get(MARKSTG.."Resetdatapost" .. msg.chat_id_) then
redis:del(MARKSTG.."Gp:Post" .. msg.chat_id_)
redis:setex(MARKSTG.."Resetdatapost" .. msg.chat_id_, 12 * hour, true)
end
----------------------com viewget -----------------------------------------------------
if redis:get(MARKSTG.."bot:viewget" .. msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, "✯↓ حدث خطا حاول مره اخرى", 1, "md")
redis:del(MARKSTG.."bot:viewget" .. msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, "📛↓ عدد المشاهدات \n: " .. msg.views_ .. "", 1, "md")
redis:del(MARKSTG.."bot:viewget" .. msg.sender_user_id_)
end end
if redis:get(MARKSTG.."bot:viewmsg") == "On" then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_) then
...
redis:del(MARKSTG..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
setphoto(msg.chat_id_, photo_id)
end
------------------------------------------------------------------------------------------------
if redis:get(MARKSTG.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local gps = redis:scard(MARKSTG.."bot:groups") or 0
local gpss = redis:smembers(MARKSTG.."bot:groups") or 0
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:del(MARKSTG.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
if redis:get(MARKSTG.."anti-flood:" .. msg.chat_id_) then
redis:setex(MARKSTG..pm, TIME_CHECK, msgs + 1)
end
--«««««««««««««««««««««««« Developer By marks MARKSTG »»»»»»»»»»»»»»»»»»»»»»»»»»»--
if msg.content_.caption_ then
...
if redis:get(MARKSTG.."bot:duplipost:mute" .. msg.chat_id_) then
if redis:sismember(MARKSTG.."Gp:Post" .. msg.chat_id_, post) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:sadd(MARKSTG.."Gp:Post" .. msg.chat_id_, post)
end
end
if redis:get(MARKSTG.."post:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.forward_info_ and redis:get(MARKSTG.."bot:forward:mute" .. msg.chat_id_) and (msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost") then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(MARKSTG.."bot:photo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:strict" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
end
if msg.content_.caption_:match("@") then
if redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityMention" and redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("#") then
if redis:get(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityHashtag" and redis:get(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and (msg.content_.entities_[0].ID == "MessageEntityTextUrl" or msg.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(MARKSTG.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.web_page_ and redis:get(MARKSTG.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\128-\191]") and redis:get(MARKSTG.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if (msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]")) and redis:get(MARKSTG.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."markdown:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."Game:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:document:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\121-\191]") and redis:get(MARKSTG.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:inline:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."clerk") == "On" or is_admin(msg.sender_user_id_) then
...
if redis:get(MARKSTG.."bot:sticker:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:tgservice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(MARKSTG.."bot:member:lock" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
...
for i = 0, #list do
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(MARKSTG.."bot:bots:mute" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(MARKSTG.."bot:botskick" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
local user_info_ = redis:get(MARKSTG..'user:Name' .. msg.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "👨‍✈️┊ العضو » (["..UserMARKSTG.."])\n🚫┊ الايدي » (*"..msg.sender_user_id_.."*)\n🗯┊ قام بأضافه بوت في المجموعه\n📌┊ تم طرد البوت مع العضو \n✓", 1, "md")
end 
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(MARKSTG.."keed_bots"..msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
redis:sadd(MARKSTG..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. MARKSTG_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(MARKSTG..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(MARKSTG.."bot:member:lock" .. msg.chat_id_) and not is_vipmem(msg.content_.members_[0].id_, msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if redis:get(MARKSTG.."bot:contact:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:music:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:voice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:location:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:video:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:selfvideo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:gifs:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_ then
check_filter_words(msg, msg.content_.caption_)
if redis:get(MARKSTG.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:setex(MARKSTG.."bot:editid" .. msg.id_, day, msg.content_.text_)
if redis:get(MARKSTG.."anti-flood:" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
redis:setex(MARKSTG..pm, TIME_CHECK, msgs + 1)
end
---------------------rem cam filters---------------------------------------------------------------------
if redis:get(MARKSTG.."Filtering:" .. msg.sender_user_id_) then
local chat = redis:get(MARKSTG.."Filtering:" .. msg.sender_user_id_)
...
---------------save name bot-----------------------------
if redis:get(MARKSTG..'botts:namess'..msg.sender_user_id_) then
redis:del(MARKSTG..'botts:namess'..msg.sender_user_id_)
local NAME_BOT = msg.content_.text_:match("(.*)")
redis:set(MARKSTG..'MARKSTGnams',NAME_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع اسم البوت 🍃",1, 'html')
return false
end
------------------------save cam link-----------------------------------
if redis:get(MARKSTG.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
local glink = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
local hash = "bot:group:link" .. msg.chat_id_
redis:set(MARKSTG..hash, glink)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم  حفظ الرابط 🎈", 1, "md")
redis:del(MARKSTG.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
--------------------------------------------------------------------------
if redis:get(MARKSTG.."gettextsec" .. msg.sender_user_id_) then
local clerktext = msg.content_.text_
redis:set(MARKSTG.."textsec", clerktext)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ الكليشه 🎈", 1, "md")
redis:del(MARKSTG.."gettextsec" .. msg.sender_user_id_)
end
---------------------save com rules-----------------------------------------------------------------------------
if redis:get(MARKSTG.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local rules = msg.content_.text_
redis:set(MARKSTG.."bot:rules" .. msg.chat_id_, rules)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ القوانين 🎋",  1, "md")
redis:del(MARKSTG.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
-----------ch join------------------------------------------------------
if redis:get(MARKSTG..'Kpch'..msg.sender_user_id_) then
redis:del(MARKSTG..'Kpch'..msg.sender_user_id_)
local url , res = https.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/getChatAdministrators?chat_id='..msg.content_.text_..'')
local data = KPJS:decode(url)
if res == 400 then
if data.description == "Bad Request: supergroup members are unavailable" then --التحقق من البوت مدير حسب الرابط اعلاه 
send(msg.chat_id_, msg.id_, 1, "👤┊ عذرا يجب عليك رفع\n©️┊ البوت مدير في القناة اولا\n✓",  1, "md")
return false 
elseif data.description == "Bad Request: chat not found" then -- التحقق من المعرف (@kpchh)
send(msg.chat_id_, msg.id_, 1, "🌀┊ خطأ هذا ليس معرف قناة",  1, "md")
return false
end end 
...
redis:set(MARKSTG..'Kpch1',CH_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع معرف القناة ❗️\n🔰┊ الان قم بتفعيل الاشتراك\n✓‏",1, 'html')
return false
end
---------------------rem cam broadcast--------------------------------------------------------------------
if redis:get(MARKSTG.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
...
--------------------rem  cam broadcast2--------------------------------------------------------------------------
if redis:get(MARKSTG.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
if text:match("^الغاء$") then
send(msg.chat_id_, msg.id_, 1, "🔰- تم الغاء الامر بنجاح🎈 ", 1, "md")
redis:del(MARKSTG.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
else
local gps = redis:scard(MARKSTG.."bot:groups") or 0
local gpss = redis:smembers(MARKSTG.."bot:groups") or 0
local msgs = {
...
if redis:get(MARKSTG.."bot:joinbylink:" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
else
end
--------------------set cam sudo  ------------------------------------------
if redis:get(MARKSTG.."bot:MARKSTG_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
redis:del(MARKSTG.."bot:MARKSTG_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
local MARKSTG_dev = msg.content_.text_:match("(.*)")
redis:set(MARKSTG.."MARKSTG_dev", MARKSTG_dev)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع كليشه المطور 🎗", 1, "md")
end
--------------------check_filter_words---------------------------------------------------------
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(MARKSTG.."bot:links:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:text:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.text_:match("[\216-\219][\121-\191]") and redis:get(MARKSTG.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(MARKSTG..hash) then
sens = 400
else
sens = tonumber(redis:get(MARKSTG..hash))
end
if redis:get(MARKSTG.."bot:spam:mute" .. msg.chat_id_) and string.len(msg.content_.text_) > sens or ctrl_chars > sens or real_digits > sens then
delete_msg(chat, msgs)
print_del_msg("Deleted Because [Lock] [Spam] ")
end
end
if (msg.content_.text_:match("[A-Z]") or msg.content_.text_:match("[a-z]")) and redis:get(MARKSTG.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(MARKSTG.."bot:cmds" .. msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
...
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' غادر$')) then
send(msg.chat_id_, msg.id_, 1, "✺↓ تم مغادره المجموعه ♩†",  1, "md")
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
end
--------------------------------------------
if text == 'تفعيل رفع المدير' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل خاصية رفع المدير\n ✓ ", 1, 'md')
redis:del(MARKSTG.."lock_addd"..msg.chat_id_)
end
if text == 'تعطيل رفع المدير' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل خاصية رفع المدير\n ✓ ", 1, 'md')
redis:set(MARKSTG.."lock_addd"..msg.chat_id_, true)
end
-------------------------------------------------
if text == 'تفعيل رفع المميز' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل خاصية رفع المميز\n ✓ ", 1, 'md')
redis:del(MARKSTG.."lock_adddvip"..msg.chat_id_)
end
if text == 'تعطيل رفع المميز' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل خاصية رفع المميز\n ✓ ", 1, 'md')
redis:set(MARKSTG.."lock_adddvip"..msg.chat_id_, true)
end
--------------------------------------------------------------------
if is_owner(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and text:match("^رفع مدير بالتفاعل (%d+)$")  then
local uuuu = { string.match(text, "^(رفع مدير بالتفاعل) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مدير\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(MARKSTG.."MARKSTG_O" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(MARKSTG.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_MARKSTG = tonumber(redis:get(MARKSTG.."MARKSTG_O" .. msg.chat_id_)) 
if msgs == get_MARKSTG and not redis:get(MARKSTG.."lock_addd"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ هذا العضو اثبت تفاعله\n🎟┊تم رفعه مدير في المجموعه\n✓",  1, "md")
redis:sadd(MARKSTG..'bot:momod:'..msg.chat_id_, msg.sender_user_id_)
end
--------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مميز\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(MARKSTG.."MARKSTG_OO" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(MARKSTG.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_MARKSTG = tonumber(redis:get(MARKSTG.."MARKSTG_OO" .. msg.chat_id_)) 
if msgs == get_MARKSTG and not redis:get(MARKSTG.."lock_adddvip"..msg.chat_id_) then
...
local user_info_ = redis:get(MARKSTG..'user:Name' .. result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(MARKSTG..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(MARKSTG..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم رفعه مميز عام\n✓', 1, 'md')
end end end
...
local hash = 'bot:vipmems:'
if result.id_ then
if redis:sismember(MARKSTG..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(MARKSTG..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
local user_info_ = redis:get(MARKSTG..'user:Name' .. ap[2])
local UserMARKSTG = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(MARKSTG..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(MARKSTG..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(MARKSTG..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(MARKSTG..hash, result.sender_user_id_)
...
local hash = 'bot:vipmems:'
if result.id_ then
if not redis:sismember(MARKSTG..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله مميز عام سابقا\n‏', 1, 'md')
else
redis:srem(MARKSTG..hash, result.id_)
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(MARKSTG..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(MARKSTG..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم حذفه من مميزين العام \n✓‏', 1, 'md')
end end
end
-----------------------------promote_by_reply-------------------------------------------------------
if text:match('^رفع مدير$') and is_owner(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ ~= 0  then
function promote_by_reply(extra, result, success)
local user_info_ = redis:get(MARKSTG..'user:Name' .. result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
...
-----------------------------promote_by_username-------------------------------------------------
if text:match('^رفع مدير @(%S+)$') and is_owner(msg.sender_user_id_, msg.chat_id_) then
...
------------------------------promote_by_ID-----------------------------------------------------------------
if text:match('^رفع مدير (%d+)$') and is_owner(msg.sender_user_id_, msg.chat_id_) then
...
-------------------------------demote_by_reply----------------------------------------------------------------------
if text:match('^تنزيل مدير$') and is_owner(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ ~= 0 then
function demote_by_reply(extra, result, success)
local user_info_ = redis:get(MARKSTG..'user:Name' .. result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
...
-------------------------demote_by_username----------------------------------------------------------------------
if text:match('^تنزيل مدير @(%S+)$') and is_owner(msg.sender_user_id_, msg.chat_id_) then
local hash = 'bot:momod:'..msg.chat_id_
...
-----------------------------demote_by_ID------------------------------------------------------------------
if text:match('^تنزيل مدير (%d+)$') and is_owner(msg.sender_user_id_, msg.chat_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
if redis:sismember(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.id_) then
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end
...
local UserMARKSTG = user_info_
if user_info_ then
if redis:sismember(MARKSTG..'bot:vipmem:'..msg.chat_id_, ap[2]) then
texts = '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(MARKSTG..'bot:vipmem:'..msg.chat_id_, ap[2])
texts = '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
send(msg.chat_id_, msg.id_, 1, '🌀┊ عذراً لا استطيع (حظر،طرد،كتم)المدراء والمدراء ❗️', 1, 'md')
else
local user_info_ = redis:get(MARKSTG..'user:Name' .. result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
...
if is_momod(result.sender_user_id_, result.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🌀┊ عذراً لا استطيع (تقييد)المدراء والمدراء ❗️', 1, 'md')
else
local user_info_ = redis:get(MARKSTG..'user:Name' .. result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'')
redis:sadd(MARKSTG..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'')
redis:sadd(MARKSTG..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'')
redis:sadd(MARKSTG..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..hash, result.sender_user_id_)
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
---------------------------setowner_by_reply----------------------------------------------------------------
if text:match('^رفع مدير عام$') and is_monshi(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ then
...
if text:match('^رفع مدير عام @(%S+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
local ap = {string.match(text, '^(رفع مدير عام) @(%S+)$')}
...
-------------------------setowner_by_ID----------------------------------------------------------------------
if text:match('^رفع مدير عام (%d+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
...
------------------------deowner_by_reply-----------------------------------------------------------------------
if text:match('^تنزيل مدير عام$') and is_monshi(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ then
...
if text:match('^تنزيل مدير عام @(%S+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
local hash = 'bot:owners:'..msg.chat_id_
local ap = {string.match(text, '^(تنزيل مدير عام) @(%S+)$')}
...
----------------------remowner_by_ID-------------------------------------------------------------------------
if text:match('^تنزيل مدير عام (%d+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
local hash = 'bot:owners:'..msg.chat_id_
...
-------------ADD ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^اضف مدير$') and is_sudo(msg) and msg.reply_to_message_id_ then
...
-------------ADD ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^اضف مدير @(%S+)$') and is_sudo(msg) then
local ap = {string.match(text, '^(اضف مدير) @(%S+)$')}
function addadmin_by_username(extra, result, success)
if result.id_ then
redis:sadd(MARKSTG..'bot:admins:', result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم اضافه مدير في البوت \n✓‏'
else
...
-------------ADD ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^اضف مدير (%d+)$') and is_sudo(msg) then
local ap = {string.match(text, '^(اضف مدير) (%d+)$')}
local hash = 'bot:admins:'
local user_info_ = redis:get(MARKSTG..'user:Name' .. ap[2])
local UserMARKSTG = user_info_
if user_info_ then
...
-------------DEL ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^حذف مدير$') and is_sudo(msg) and msg.reply_to_message_id_ then
...
-------------DEL ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^حذف مدير @(%S+)$') and is_sudo(msg) then
local hash = 'bot:admins:'
local ap = {string.match(text, '^(حذف مدير) @(%S+)$')}
...
-------------DEL ADMIN FROM BOT----------------------------------------------------------------------------------
if text:match('^حذف مدير (%d+)$') and is_sudo(msg) then
local ap = {string.match(text, '^(حذف مدير) (%d+)$')}
local hash = 'bot:admins:'
local user_info_ = redis:get(MARKSTG..'user:Name' .. ap[2])
local UserMARKSTG = user_info_
if user_info_ then
...
redis:srem(MARKSTG..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تنزيله من المطورين \n✓‏', 1, 'md')
table.remove(_config.Sudo_Users, getindex(_config.Sudo_Users, marks))
save_on_config()
load_config()
end
end
...
-------------------------------------------------------------
if text:match('^تجريد الرتب$') and is_monshi(msg.sender_user_id_, msg.chat_id_) and msg.reply_to_message_id_ then
function delallrtb(extra, result, success)
local user_info_ = redis:get(MARKSTG..'user:Name' ..result.sender_user_id_)
local UserMARKSTG = user_info_
if user_info_ then
local k1 = not redis:sismember(MARKSTG..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(MARKSTG..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(MARKSTG..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
redis:srem(MARKSTG..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
redis:srem(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تجريد الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
------------------------------------------------------------------------
if text:match('^تجريد الرتب @(%S+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
local ap = {string.match(text, '^(تجريد الرتب) @(%S+)$')}
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(MARKSTG..'bot:owners:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(MARKSTG..'bot:momod:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(MARKSTG..'bot:owners:'..msg.chat_id_, result.id_)
redis:srem(MARKSTG..'bot:momod:'..msg.chat_id_, result.id_)
redis:srem(MARKSTG..'bot:vipmem:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تجريد الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
--------------------------------------------------------------------
if text:match('^تجريد الرتب (%d+)$') and is_monshi(msg.sender_user_id_, msg.chat_id_) then
local ap = {string.match(text, '^(تجريد الرتب) (%d+)$')}
local user_info_ = redis:get(MARKSTG..'user:Name' ..ap[2])
local UserMARKSTG = user_info_
if user_info_ then
local k1 = not redis:sismember(MARKSTG..'bot:owners:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(MARKSTG..'bot:momod:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(MARKSTG..'bot:vipmem:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(MARKSTG..'bot:owners:'..msg.chat_id_, ap[2])
redis:srem(MARKSTG..'bot:momod:'..msg.chat_id_, ap[2])
redis:srem(MARKSTG..'bot:vipmem:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم تجريد الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserMARKSTG = user_info_
if user_info_ then
local k1 = not redis:sismember(MARKSTG..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(MARKSTG..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(MARKSTG..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
redis:srem(MARKSTG..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
redis:srem(MARKSTG..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(MARKSTG..'bot:keed:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(MARKSTG..'bot:muted:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(MARKSTG..'bot:banned:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..'bot:keed:'..msg.chat_id_, result.id_)
redis:srem(MARKSTG..'bot:muted:'..msg.chat_id_, result.id_)
redis:srem(MARKSTG..'bot:banned:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
local UserMARKSTG = user_info_
if user_info_ then
local k1 = not redis:sismember(MARKSTG..'bot:keed:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(MARKSTG..'bot:muted:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(MARKSTG..'bot:banned:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(MARKSTG..'bot:keed:'..msg.chat_id_, ap[2])
redis:srem(MARKSTG..'bot:muted:'..msg.chat_id_, ap[2])
redis:srem(MARKSTG..'bot:banned:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserMARKSTG..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ معرفك : [@" .. ust .. "]", 1, "md")
else
...
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserMARKSTG..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( الاسرع ) للعب\n✓',1, 'md')
return false end end
if text == 'الاسرع'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(MARKSTG..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'') then
...
💬┊لعضو مميز ارسل رقم ( *1* ) 
 ✓
]]
redis:set(MARKSTG..'marks1'..msg.sender_user_id_..''..msg.chat_id_..'','marks2')
send(msg.chat_id_, msg.id_, 1, keep,1, 'md')
return false end end end
if text then
local MARKSTG1 = redis:get(MARKSTG..'marks1'..msg.sender_user_id_..''..msg.chat_id_..'')
if MARKSTG1 == 'marks2' then
if text == '1' then
...
redis:del(MARKSTG..'marks1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numkep = tonumber(redis:get(MARKSTG..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل امر اطردني\n ✓ ", 1, 'md')
redis:set(MARKSTG.."lock_kickme"..msg.chat_id_, true)
return false 
end
if text == 'تعطيل اطردني' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل امر اطردني\n ✓ ", 1, 'md')
redis:del(MARKSTG.."lock_kickme"..msg.chat_id_)
return false 
end
-------------------add ch --------------------------
...
redis:setex(MARKSTG..'Kpch'..msg.sender_user_id_,300,true)
send(msg.chat_id_, msg.id_, 1, "🌀┊ ارسل لي معرف قناتك 🍃\n",1, 'html')
end end
...
if not redis:get(MARKSTG..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(MARKSTG..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(MARKSTG.."Kpjoin1", true)
return false end end end
...
redis:del(MARKSTG.."dellmssg"..msg.chat_id_)
end
----------------ADD DEL MSG------------------------------------------------------------
...
redis:set(MARKSTG.."dellmssg"..msg.chat_id_, true)
end
----------add - rem bot free----------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل البوت خدمي\n ✓ ", 1, 'md')
redis:set(MARKSTG.."bot:free", true)
end end
...
local UserMARKSTG = user_info_
if user_info_ then
local start = redis:get(MARKSTG.."startbot")
if start then
keep = [[
[]]..start..[[]
]]
...
💠┊مرحبا انا بوت اسمي *(]]..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..[[)🗼*
💢┊اقوم بحماية مجموعتك  مُْْْن '
📜┊الروابط، والتكرار، السبام وغيرها '
🔰┊قم بأضافة  البوت الى المجموعه ،
🌀┊ثم ارسل (تفعيل) او راسل المطور،
✔️┊لكي يتم تفعيله فْي المجموعِْْه'
🔱┊*مطور البوت* » ( []] .. UserMARKSTG .. [[] )‏
‏
‏]]
send(msg.chat_id_, msg.id_, 1, MARKSTG, 1, 'md')
end end end
-----------------------get start------------------------------------------------------------------------
...
local Kpch2 = redis:get(MARKSTG.."Kpch1")
if Kpch2 then
...
------------------list Admins BOT--------------------------------------------------------------------------
if text:match("^مدراء البوت$") then
if not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '💲┊ للمطورين فقط', 1, 'md')
else
local hash = "Bot:Admins"
...
text = "🌀┊  مدراء البوت :\n*≖≖≖≖≖≖≖≖≖≖≖≖*\n"
for k, v in pairs(list) do
if redis:get(MARKSTG.."SudoNumberGp" .. v) then
gps = tonumber(redis:get(MARKSTG.."SudoNumberGp" .. v))
else
gps = 0
end
local user_info = redis:get(MARKSTG.."user:Name" .. v)
...
local time = a[2] * day
redis:setex(MARKSTG.."bot:charge:"..msg.chat_id_,time,true)
redis:set(MARKSTG.."bot:enable:"..msg.chat_id_,true)
end
------------------charge-----------------------------------------------------------------------------
if text:match("^فحص الشحن") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local ex = redis:ttl(MARKSTG.."bot:charge:"..msg.chat_id_)
if ex == -1 then
...
local ex = redis:ttl(MARKSTG.."bot:charge:"..txt[2])
if ex == -1 then
...
if not redis:get(MARKSTG.."bot:free") and not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '🚫┊* للمطورين فقط* ❗️', 1, 'md')
return false end
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(MARKSTG..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
if result.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
redis:sadd(MARKSTG.."bot:owners:"..msg.chat_id_,owner_id)
end
end
end
getChannelMembers(msg.chat_id_, 0, 'Administrators', 200, promote_admin)
redis:set(MARKSTG.."test:group"..msg.chat_id_,'krar')
if redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🚫┊ المجموعه  مفعله سابقا ❗️\n‏\n', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🚫┊ تم تفعيل المجموعه\n🔱┊ وتم رفع المدراء ومدير عام عام ❗️\n‏\n', 1, 'md')
end
if result.first_name_ then
...
if not redis:get(MARKSTG.."bot:group:link"..msg.chat_id_) then
local getlink = 'https://api.telegram.org/bot'..MARKSTG_TOKEN..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(MARKSTG.."bot:group:link"..msg.chat_id_,link.result)
end
end
local lik_1 = redis:get(MARKSTG.."bot:group:link"..msg.chat_id_)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(Kp_Owner,0, 1,"- *تم اضافه مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ مدير عام عام ≈ [" .. owner .. "]\n🔰┊ ["..title_name(msg.chat_id_).."]("..(link or "t.me/MARKSTG_ch")..")\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " ..result.first_name_.. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n✓", 1, "md")
redis:set(MARKSTG.."bot:enable:" .. msg.chat_id_, true)
redis:setex(MARKSTG.."bot:charge:" .. msg.chat_id_, 9999 * day, true)
redis:sadd(MARKSTG.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
end
tdcli_function ({
ID = "GetChannelFull",
...
redis:set(MARKSTG.."bot:charge:"..txt[2],true)
...
local txt = { string.match(text, "^(تفعيل)$")}
redis:del(MARKSTG..'lock:add'..msg.chat_id_)
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(MARKSTG..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
redis:set(MARKSTG..'lock:add'..msg.chat_id_,true)
if not redis:get(MARKSTG.."bot:enable:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🎟┊ المجموعه معطله اساسا\n', 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, '🎟┊ تم تعطيل المجموعه\n', 1, 'md')
end
redis:del(MARKSTG.."bot:enable:" .. msg.chat_id_)
redis:srem(MARKSTG.."bot:groups", msg.chat_id_)
redis:del(MARKSTG.."test:group"..msg.chat_id_)
redis:srem(MARKSTG.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ مدير عام عام ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغط هنا](" .. (redis:get(MARKSTG.."bot:group:link"..msg.chat_id_) or "https://t.me/MARKSTG_ch") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local gp = txt[3]
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم تفعيل المجموعه \n(*"..txt[2].."*) يوم 🍃", 1, 'html')
redis:sadd(MARKSTG..'sudo:data:'..sudo, gp)
end
-------------------REEM----------------------------------------------------------------------------
...
local send_to_bot_owner = function(extra, result)
redis:del(MARKSTG.."bot:enable:" .. gp[2])
redis:del(MARKSTG.."bot:charge:" .. gp[2])
local v = tonumber(Kp_Owner)
local fname = result.first_name_ or ""
local lname = result.last_name_ or ""
local username = "@" .. result.username_ or ""
...
redis:srem(MARKSTG.."sudo:data:" .. msg.sender_user_id_, gp[2])
redis:srem(MARKSTG.."bot:groups", gp[2])
end
getUser(msg.sender_user_id_, send_to_bot_owner)
end
else
...
local num_keep = (tonumber(redis:get(MARKSTG.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(MARKSTG.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local Kpcontact = (tonumber(redis:get(MARKSTG.."kpaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
...
marks_info2 = "🎟┊ ايديه  » `" .. result.id_ .. "`\n©️┊ معرفه »  [@" .. ap[2] .. "]\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. t .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
elseif not result.id_ then
marks_info2 = "🌀┊ لا يوجد عضو بهذا المعرف"
end
send(msg.chat_id_, msg.id_, 1, marks_info2, 1, "md")
end
...
local UserMARKSTG = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "•اهلا ~ ["..UserMARKSTG.."]\n🎧» ايديك  (`"..msg.sender_user_id_.."`)\n✓", 1, "md")
return false end end
-------------------RETBA-------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local num_keep = tonumber(redis:get(MARKSTG..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(MARKSTG.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Kpcontact = (tonumber(redis:get(MARKSTG.."kpaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
local marks_info = "🎟┊ ايديك  » " .. msg.sender_user_id_ .. "\n©️┊ معرفك »  " .. username .. "\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🔆┊ نقاطك  »   "..num_keep.."\n🌐┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
local marks_info2 = "🎟┊ ايديك  » `" .. msg.sender_user_id_ .. "`\n©️┊ معرفك »  [" .. username .. "]\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🔆┊ نقاطك  »   "..num_keep.."\n🌐┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"

if redis:get(MARKSTG.."getidstatus" .. msg.chat_id_) == "Photo" then
if result.photos_[0] then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_, marks_info)
else
send(msg.chat_id_, msg.id_, 1, "انت لا تمتلك صوره لحسابك🎈‏\n"..marks_info2.."", 1, "md")
end
end
if redis:get(MARKSTG.."getidstatus" .. msg.chat_id_) == "Simple" then
send(msg.chat_id_, msg.id_, 1, marks_info2, 1, "md")
end
if not redis:get(MARKSTG.."getidstatus" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, marks_info2, 1, "md")
 end 
 end
tdcli_function({
...
--------------------Kp concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '🔰┊ عدد جهاتك ≈ * '..(tonumber(redis:get(MARKSTG..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function iD_reP(extra, result, success)
local num_keep = (tonumber(redis:get(MARKSTG.."incr_msg"..result.sender_user_id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(MARKSTG.."msgs:"..result.sender_user_id_..":"..msg.chat_id_) or 0 ))
local Kpcontact = (tonumber(redis:get(MARKSTG.."kpaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Kp_Owner) then
MARKSTG3 = "مطور الاساسي 🌿"
elseif is_sudoid(result.sender_user_id_) then
MARKSTG3 = "المطور 🐾"
elseif is_admin(result.sender_user_id_) then
MARKSTG3 = "مدير في البوت"
elseif is_vipmems(result.sender_user_id_) then
MARKSTG3 = "مميز عام 🌿"
elseif is_monshi(result.sender_user_id_, msg.chat_id_) then
MARKSTG3 = "منشىء الكروب 🐾"
elseif is_owner(result.sender_user_id_, msg.chat_id_) then
MARKSTG3 = "مدير عام عام 🌿"
elseif is_momod(result.sender_user_id_, msg.chat_id_) then
MARKSTG3 = "مدير المجموعه🐾"
...
local MARKSTG_info = "🎟┊ ايديه  » `" .. result.sender_user_id_ .. "`\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. MARKSTG3 .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
send(msg.chat_id_, result.id_, 1, MARKSTG_info, 1, "md")
end
...
redis:setex(MARKSTG..'botts:namess'..msg.sender_user_id_,698,true)
...
if redis:get(MARKSTG.."getpro:" .. msg.chat_id_) == "Active" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
redis:set(MARKSTG.."getpro:" .. msg.chat_id_, "Active")
end end end
...
if redis:get(MARKSTG.."getpro:" .. msg.chat_id_) == "Deactive" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
redis:set(MARKSTG.."getpro:" .. msg.chat_id_, "Deactive")
end end end
---------------------------------------------------------------------
if text:match("^مسح الصوره") and is_momod(msg.sender_user_id_, msg.chat_id_)  then
https.request('https://api.telegram.org/bot'..MARKSTG_TOKEN..'/deleteChatPhoto?chat_id='..msg.chat_id_)
...
if not is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(MARKSTG.."getpro:" .. msg.chat_id_) == "Deactive" then
...
redis:set(MARKSTG..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '🌀┊ تم وضع  التكرار : '..floodt[2]..'', 1, 'md')
end end
----------------------floodstatus----------------------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التكرار بالطرد\n ✓ ", 1, 'md')
redis:set(MARKSTG.."floodstatus" .. msg.chat_id_, "Kicked")
end
end
if status[2] == "بالمسح" then
if redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) == "DelMsg" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
redis:set(MARKSTG.."floodstatus" .. msg.chat_id_, "DelMsg")
end
end
end
-------------------SET warn-----------------------------------------
...
redis:set(MARKSTG.."warn:max:" .. msg.chat_id_, warnmax[2])
end
end
---------------------warnstatus----------------------------------------------------------
...
if redis:get(MARKSTG.."warnstatus" .. msg.chat_id_) == "Muteuser" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
redis:set(MARKSTG.."warnstatus" .. msg.chat_id_, "Muteuser")
end   end
if status[2] == "بالطرد" then
if redis:get(MARKSTG.."warnstatus" .. msg.chat_id_) == "Remove" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
redis:set(MARKSTG.."warnstatus" .. msg.chat_id_, "Remove")
end end end
-----------------------getidstatus-----------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(MARKSTG.."getidstatus" .. msg.chat_id_, "Photo")
end end  end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(MARKSTG.."getidstatus" .. msg.chat_id_, "Simple")
end end end
-------------------autoleave-----------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تفعيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(MARKSTG.."autoleave", "On")
end end end
if is_sudo(msg) and text:match("^تعطيل (.*)$") then
local status = { string.match(text, "^(تعطيل) (.*)$")}
if status[2] == "الخروج التلقائي" then
if redis:get(MARKSTG.."autoleave") == "Off" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(MARKSTG.."autoleave", "Off")
end   end  end
-----------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "🌀┊ ارسل لي كليشه المطور الان 🎗", 1, "md")
redis:setex(MARKSTG.."bot:MARKSTG_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 370, true)
end end
...
local MARKSTG_dev = redis:get(MARKSTG.."MARKSTG_dev")
if MARKSTG_dev then
send(msg.chat_id_, msg.id_, 1, "["..MARKSTG_dev.."]", 1, "md")
else
local marks = [[
...
📌┊ قم بأضافة البوت  وارفعه مدير 
🗯┊ ثم ارسل كلمه *(تفعيل)*
🎟┊ سيتم رفع المدراء والمنشىء 

⚜️» مطور البوت » []]..UserMARKSTG..[[]
〰
]]
send(msg.chat_id_, msg.id_, 1, marks, 1, "md") 
end
end
end
getUser(msg.sender_user_id_, get_me)
...
redis:del(MARKSTG.."MARKSTG_dev")
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم حذف كليشه المطور\n ✓ ", 1, 'md')
end
end
-------------------------SET LINK---------------------------------------------------------------
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^ضع رابط$"))  then
send(msg.chat_id_, msg.id_, 1, "🌀┊ ارسل لي الرابط الان 🎐", 1, "md")
redis:setex(MARKSTG.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 1200, true)
end
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^حذف الرابط$"))  then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم حذف الرابط بنجاح\n ✓ ", 1, 'md')
redis:del(MARKSTG.."bot:group:link" .. msg.chat_id_)
end
if text:match("^الرابط$") then
local link = redis:get(MARKSTG.."bot:group:link" .. msg.chat_id_)
if link then
send(msg.chat_id_, msg.id_, 1, "📬¦ رابط المجموعه :\n" .. link .. "", 1, "md")
else
...
redis:setex(MARKSTG.."Filtering:" .. msg.sender_user_id_, 80, msg.chat_id_)
end
----------------------------------------------------------------------------------
...
local hash = "bot:filters:" .. msg.chat_id_
if not redis:hget(MARKSTG..hash, name) then
send(msg.chat_id_, msg.id_, 1, "🌀┊  الكلمه〖  " .. name .. "  〗\n👥┊  تم الغاء منعها📍", 1, "md")
redis:hdel(MARKSTG..hash, name)
end end
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_KP(msg) then
local users = redis:scard(MARKSTG.."bot:userss")
local botnamess = redis:get(MARKSTG.."MARKSTGnams") or "كيبر"
...
local get_me = function(extra, result)
local num = (redis:scard(MARKSTG.."bot:groups"))
local list = redis:smembers(MARKSTG.."bot:groups")
local text = "~ All_Groups_Bots \n\n"
for k,v in pairs(list) do
local GroupsOwner = redis:scard(MARKSTG.."bot:owners:"..v) or 0
local GroupsMod = redis:scard(MARKSTG.."bot:momod:"..v) or 0
local Groupslink = redis:get(MARKSTG.."bot:group:link" ..v)
if result.first_name_ then
if #result.first_name_ < 35 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
file:write(text)
file:close()
local token_files = 'https://api.telegram.org/bot' .. MARKSTG_TOKEN .. '/sendDocument'
...
local botnamess = redis:get(MARKSTG.."MARKSTGnams") or "كيبر"
local kpSudos1 = redis:scard(MARKSTG.."Bot:KpSudos") or "0"
local kpAdmins = redis:scard(MARKSTG.."Bot:Admins") or "0"
local kpgban = redis:scard(MARKSTG.."bot:gban:") or "0"
local kpvipmems = redis:scard(MARKSTG.."bot:vipmems:") or "0"
local kpgps = redis:scard(MARKSTG.."bot:groups") or "0"
local kpusers = redis:scard(MARKSTG.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..kpSudos1.."*\n🔰┊عدد المدراء ≈ *"..kpAdmins.."*\n🔚┊عدد الكروبات ≈ * "..kpgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. kpusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..kpgban.."*\n🔘┊عدد المميزين عام ≈ * "..kpvipmems.."*\n🗯┊قناة السورس ≈ [MARKSTG](t.me/MARKSTG_ch)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
if text:match("^المجموعات$") or text:match("^الكروبات$") and is_admin(msg.sender_user_id_, msg.chat_id_) then
local gps = redis:scard(MARKSTG.."bot:groups")
local users = redis:scard(MARKSTG.."bot:userss")
...
function cb(a,b,c)
local text = b.content_.text_
local users = redis:scard(MARKSTG.."bot:userss") or 0
local userss = redis:smembers(MARKSTG.."bot:userss") or 0
local msgs = {
[0] = id
}
for i = 1, #userss do
...
if redis:get(MARKSTG.."ffosh"..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..MARKSTG3.."*) 🍃\n‏" , 1, "md")
end
...
`git clone https://github.com/alqaser/MARKSTG.git && cd MARKSTG && chmod +x MARKSTG && ./MARKSTG`

🔱┊ *راح يطلب ايديك كمطور
💲┊ بعدها يطلب التوكن*
يشتغل تلقائيا ميحتاج تسوي
سكرين ....
📮┊ *كود التشغيل :❗️*

`killall screen && cd MARKSTG && screen ./MARKSTG`

💬┊ *مطور السورس *≈ [@rr20r](t.me/rr20r)
💲┊ *قناٌة السوٰرس *≈ [@MARKSTG_ch](T.ME/MARKSTG_ch)
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيه فقط', 1, 'md')
else
local text = [[
  🚦¦ مرحبا عزيزي في قائمة الاوامر
  ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
  ☑️¦(م1) ~⊱ لعرض اوامر الحمايه
  🔘¦(م2) ~⊱ لعرض اوامر المدراء العامين
  👁‍🗨¦(م3) ~⊱ لعرض اوامر المدراء
  🚸¦(م4) ~⊱ لعرض اوامر المطور الثانوي
  💬¦(م5) ~⊱ لعرض الاوامر الجانبيه
  💻¦(م6) ~⊱ لعرض اوامر المطور الاساسي
  ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
  قناتنا ~» @MARKS_TEAM
‌]]
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
...
local text = [[
  🚦¦مرحباعزيزي في قائمةاوامرالحمايه
  ┉┉┉┉┉┉┉┉┉
  🔐┇قفل/فتح
...
local text = [[
  🚦┇اوامر المدراء العامين والمنشئين* »
...
redis:set(MARKSTG..'welcome:'..msg.chat_id_,welcome)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم وضع الترحيب\n ✓ ", 1, 'md')
end
-----------------welcome------------------------------------------
if text:match("^الترحيب$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local wel = redis:get(MARKSTG..'welcome:'..msg.chat_id_)
if wel then
...
redis:setex(MARKSTG.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
redis:setex(MARKSTG.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊  ارسل لي الرساله الان📲\n🚫┊ للخروج ارسل لي الغاء 📍 ", 1, "md")
end end end
----------------- info bot --------------------------------------------------------------
...
local gps = redis:scard(MARKSTG.."bot:groups")
local users = redis:scard(MARKSTG.."bot:userss")
local allmgs = redis:get(MARKSTG.."bot:allmsgs")
if redis:get(MARKSTG.."autoleave") == "On" then
autoleaveAR = "مفعل"
elseif redis:get(MARKSTG.."autoleave") == "Off" then
autoleaveAR = "معطل"
elseif not redis:get(MARKSTG.."autoleave") then
autoleaveAR = "معطل"
end
if redis:get(MARKSTG.."joinbylink") == "On" then
joinbylinkAR = "مفعل"
elseif redis:get(MARKSTG.."joinbylink") == "Off" then
joinbylinkAR = "معطل"
elseif not redis:get(MARKSTG.."joinbylink") then
joinbylinkAR = "معطل"
end
...
Uptime_1 = redis:get(MARKSTG.."bot:Uptime")
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local lockMARKSTG = {string.match(text, "^(قفل) (.*)$")}
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيه فقط', 1, 'md')
else
if lockMARKSTG[2] == "التعديل" then
if not redis:get(MARKSTG.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التعديل \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG..'editmsg'..msg.chat_id_,'delmsg')
end
if lockMARKSTG[2] == "الاوامر" then
if not redis:get(MARKSTG.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الاوامر \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:cmds" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "البوتات" then
if not redis:get(MARKSTG.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:bots:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "البوتات بالطرد" then
if not redis:get(MARKSTG.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊البوتات بالطرد مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:botskick" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "التكرار" then
if not redis:get(MARKSTG.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التكرار \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."anti-flood:" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "التثبيت" then
if not redis:get(MARKSTG.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التثبيت \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:pin:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الكل" then
if not redis:get(MARKSTG.."bot:muteall" .. msg.chat_id_) then
...
redis:set(MARKSTG.."bot:bots:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."anti-flood:" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:photo:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:video:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:selfvideo:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:document:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."markdown:lock" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:gifs:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:music:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:voice:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."tags:lock" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:contact:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:tgservice:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."bot:forward:mute" .. msg.chat_id_, true)
redis:set(MARKSTG.."ffosh"..msg.chat_id_, true)
end
if lockMARKSTG[2] == "الدردشه" then
if not redis:get(MARKSTG.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الدردشه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:text:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الصور" then
if not redis:get(MARKSTG.."bot:photo:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "المشاركه" then
if not redis:get(MARKSTG.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل المشاركه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:duplipost:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الكلايش" then
if not redis:get(MARKSTG.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الكلايش \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:spam:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الفيديو" then
if not redis:get(MARKSTG.."bot:video:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "السيلفي" then
if not redis:get(MARKSTG.."bot:selfvideo:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الفايلات" then
if not redis:get(MARKSTG.."bot:document:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الالعاب" then
if not redis:get(MARKSTG.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الالعاب \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."Game:lock" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الانلاين" then
if not redis:get(MARKSTG.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانلاين \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:inline:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "البوست" then
if not redis:get(MARKSTG.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوست \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."post:lock" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الماركدون" then
if not redis:get(MARKSTG.."markdown:lock" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "المتحركه" then
if not redis:get(MARKSTG.."bot:gifs:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الاغاني" then
if not redis:get(MARKSTG.."bot:music:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الصوت" then
if not redis:get(MARKSTG.."bot:voice:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الروابط" then
if not redis:get(MARKSTG.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الروابط \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:links:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "المواقع" then
if not redis:get(MARKSTG.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل المواقع \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:location:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "التاك" then
if not redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الهاشتاك" then
if not redis:get(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الهاشتاك \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الجهات" then
if not redis:get(MARKSTG.."bot:contact:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الويب" then
if not redis:get(MARKSTG.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الويب \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:webpage:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "العربيه" then
if not redis:get(MARKSTG.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل العربيه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:arabic:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الانظمام" then
if not redis:get(MARKSTG.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانظمام \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:member:lock" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الانكليزيه" then
if not redis:get(MARKSTG.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانكليزي \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:english:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "الاشعارات" then
if not redis:get(MARKSTG.."bot:tgservice:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الملصقات" then
if not redis:get(MARKSTG.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الملصقات \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:set(MARKSTG.."bot:sticker:mute" .. msg.chat_id_, true)
end
if lockMARKSTG[2] == "التوجيه" then
if not redis:get(MARKSTG.."bot:forward:mute" .. msg.chat_id_) then
...
if lockMARKSTG[2] == "الفشار" then
if not redis:get(MARKSTG.."ffosh" .. msg.chat_id_) then
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local UNMARKSTG = { string.match(text, "^(فتح) (.*)$") }
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيه فقط', 1, 'md')
else
if UNMARKSTG[2] == "التعديل" then
if redis:get(MARKSTG.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التعديل \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
redis:del(MARKSTG.."editmsg" .. msg.chat_id_)
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
end
if UNMARKSTG[2] == "الاوامر" then
if redis:get(MARKSTG.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الاوامر \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:cmds" .. msg.chat_id_)
end
if UNMARKSTG[2] == "البوتات" then
if redis:get(MARKSTG.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:bots:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "البوتات بالطرد" then
if redis:get(MARKSTG.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات بالطرد مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:botskick" .. msg.chat_id_)
end
if UNMARKSTG[2] == "التكرار" then
if redis:get(MARKSTG.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التكرار \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."anti-flood:" .. msg.chat_id_)
end
if UNMARKSTG[2] == "التثبيت" then
if redis:get(MARKSTG.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التثبيت \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:pin:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الكل" then
if redis:get(MARKSTG.."bot:muteall" .. msg.chat_id_) then
...
redis:del(MARKSTG.."bot:bots:mute" .. msg.chat_id_)
redis:del(MARKSTG.."anti-flood:" .. msg.chat_id_)
redis:del(MARKSTG.."bot:photo:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:video:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:selfvideo:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:document:mute" .. msg.chat_id_)
redis:del(MARKSTG.."markdown:lock" .. msg.chat_id_)
redis:del(MARKSTG.."bot:gifs:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:music:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:voice:mute" .. msg.chat_id_)
redis:del(MARKSTG.."tags:lock" .. msg.chat_id_)
redis:del(MARKSTG.."bot:contact:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:tgservice:mute" .. msg.chat_id_)
redis:del(MARKSTG.."bot:forward:mute" .. msg.chat_id_)
redis:del(MARKSTG.."ffosh"..msg.chat_id_)
end
if UNMARKSTG[2] == "الدردشه" then
if redis:get(MARKSTG.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الدردشه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:text:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الصور" then
if redis:get(MARKSTG.."bot:photo:mute" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "المشاركه" then
if redis:get(MARKSTG.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح المشاركه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:duplipost:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الكلايش" then
if redis:get(MARKSTG.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الكلايش \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:spam:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الفيديو" then
if redis:get(MARKSTG.."bot:video:mute" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الالعاب" then
if redis:get(MARKSTG.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الالعاب \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."Game:lock" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الانلاين" then
if redis:get(MARKSTG.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانلاين \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:inline:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "البوست" then
if redis:get(MARKSTG.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوست \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")            
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."post:lock" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الماركدون" then
if redis:get(MARKSTG.."markdown:lock" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الروابط" then
if redis:get(MARKSTG.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الروابط \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:links:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "المواقع" then
if redis:get(MARKSTG.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح المواقع \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:location:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "التاك" then
if redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الهاشتاك" then
if redis:get(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الهاشتاك \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:hashtag:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الجهات" then
if redis:get(MARKSTG.."bot:contact:mute" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الويب" then
if redis:get(MARKSTG.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الويب \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:webpage:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "العربيه" then
if redis:get(MARKSTG.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح العربيه \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:arabic:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الانظمام" then
if redis:get(MARKSTG.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانظمام \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مفتوح سابقا\n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:member:lock" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الانكليزيه" then
if redis:get(MARKSTG.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانكليزي \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:english:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "الاشعارات" then
if redis:get(MARKSTG.."bot:tgservice:mute" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الملصقات" then
if redis:get(MARKSTG.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الملصقات \n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مفتوحه سابقا\n🎟┊ الأمر بواسطه » "..tmMARKSTG(msg).."\n ‏ ", 1, "md")
end
redis:del(MARKSTG.."bot:sticker:mute" .. msg.chat_id_)
end
if UNMARKSTG[2] == "التوجيه" then
if redis:get(MARKSTG.."bot:forward:mute" .. msg.chat_id_) then
...
if UNMARKSTG[2] == "الفشار" then
if redis:get(MARKSTG.."ffosh"..msg.chat_id_) then
...
redis:del(MARKSTG.."keed_text" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_photo" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_video" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_Document" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_markdon" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_gif" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_audio" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_voice" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_link" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_user" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_hashtag" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_contect" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_arbic" .. msg.chat_id_)
end
...
redis:del(MARKSTG.."keed_fosh" .. msg.chat_id_)
end
...
redis:set(MARKSTG.."bot:sens:spam" .. msg.chat_id_, sensspam[2])
...
local kpbanned = redis:scard(MARKSTG.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(MARKSTG.."bot:banned:" .. msg.chat_id_)
...
send(msg.chat_id_, msg.id_, 1, "🌀┊ اهلاً عزيزي ...؟🕵🏻\n🚸┊  اذا اردت مسحهم\n👷┊ ارسل لي رقم (1) \n🔍┊ او ارسل (2) لأضافتهم.️\n‏",  1, "md")
redis:setex(MARKSTG.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 35, true)
end end
----------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "🔖↓ تم ✔️ مسح البوتات ♬♩", 1, "md")
...
send(msg.chat_id_, msg.id_, 1, '💲┊ للمدراء فقط', 1, 'md')
else
local kpmomod = redis:scard(MARKSTG.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد الأدمنيه ≈ *"..kpmomod.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(MARKSTG.."bot:momod:" .. msg.chat_id_)
end end
-----------------------------------------------
if txts[2] == "المدراء" and idf:match("-100(%d+)") then
...
local kpowners = redis:scard(MARKSTG.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المدراء ≈ *"..kpowners.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(MARKSTG.."bot:owners:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local kpmonshis = redis:scard(MARKSTG.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المنشئين ≈ *"..kpmonshis.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(MARKSTG.."bot:monshis:" .. msg.chat_id_)
end end
--------DEL MARKSTG_SUDO-----------------------------------------
...
--------------DEL Admins------------------------------------
if txts[2] == "مدراء البوت" then
if not is_KP(msg) then
send(msg.chat_id_, msg.id_, 1, '💲┊ للمطور الاساسي فقط', 1, 'md')
else
local kpAdmins = redis:scard(MARKSTG.."Bot:Admins") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد مدراء البوت ≈ *"..kpAdmins.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(MARKSTG.."Bot:Admins")
end end
-----------DEL vipmem-----------------------------------------------
...
local kpvipmem = redis:scard(MARKSTG.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين ≈ *"..kpvipmem.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(MARKSTG.."bot:vipmem:" .. msg.chat_id_)
end  end
---------------keeed del --------------------------------------------
...
local kpkeed = redis:scard(MARKSTG.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المقيدين ≈ *"..kpkeed.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(MARKSTG..hash)
for k, v in pairs(list) do
redis:del(MARKSTG.."bot:keed:" .. msg.chat_id_)
HTTPS.request("https://api.telegram.org/bot"..MARKSTG_TOKEN.."/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..v.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
end end end
------------DEL vipmems-------------------------------------------
...
local kpmuted = redis:scard(MARKSTG.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(MARKSTG.."bot:muted:" .. msg.chat_id_)
...
if redis:get(MARKSTG.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local d = 0
if text:match("^1$") then
redis:del(MARKSTG.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
for i = 1, NumberReturn do
getChannelMembers(msg.chat_id_, d, "Kicked", 200, kickedlist)
d = d + 200
end
if redis:get(MARKSTG.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(MARKSTG.."flood:max:" .. msg.chat_id_) then
flood_m = 5
else
flood_m = redis:get(MARKSTG.."flood:max:" .. msg.chat_id_)
end
if not redis:get(MARKSTG.."bot:sens:spam" .. msg.chat_id_) then
spam_c = 400
else
spam_c = redis:get(MARKSTG.."bot:sens:spam" .. msg.chat_id_)
end
if redis:get(MARKSTG.."warn:max:" .. msg.chat_id_) then
sencwarn = tonumber(redis:get(MARKSTG.."warn:max:" .. msg.chat_id_))
else
sencwarn = 4
end
if redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) == "DelMsg" then
floodstatus = "المسح"
elseif redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) == "Kicked" then
floodstatus = "الطرد"
elseif not redis:get(MARKSTG.."floodstatus" .. msg.chat_id_) then
floodstatus = "الطرد"
end
if redis:get(MARKSTG.."warnstatus" .. msg.chat_id_) == "Muteuser" then
warnstatus = "الكتم"
elseif redis:get(MARKSTG.."warnstatus" .. msg.chat_id_) == "Remove" then
warnstatus = "الطرد"
elseif not redis:get(MARKSTG.."warnstatus" .. msg.chat_id_) then
warnstatus = "الكتم"
end
if redis:get(MARKSTG.."bot:music:mute" .. msg.chat_id_) then
mute_music = "✅"
...
os.execute('wget https://raw.githubusercontent.com/alqaser/MARKSTG/master/MARKSTG.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('MARKSTG.lua')  
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
redis:setex(MARKSTG.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
redis:set(MARKSTG..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if result.id_ then
send(msg.chat_id_, msg.id_, 1, "🚺  المجموعۿ : \n- " .. result.title_ .. "\n📛 تم اخراج البوت منها💯", 1, "md")
if redis:get(MARKSTG.."lang:gp:" .. result.id_) then
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserMARKSTG .. "]\n‏", 1, "md")
else
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserMARKSTG .. "]\n‏", 1, "md")
end
chat_leave(result.id_, bot_id)
redis:srem(MARKSTG.."bot:groups", result.id_)
else
...
if redis:get(MARKSTG.."bot:reloadingtime") then
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم تحديث البوت 🎐 ", 1, "md")
else
getUser(txt[2], get_data)
end end end
----------INFO MARKSTG_SUDO BY REPLY----------------------------------------------------------------------------------------------------
...
local gp = tonumber(redis:scard(MARKSTG.."bot:userss"))
gps = redis:smembers(MARKSTG.."bot:userss")
...
----------------promote_admin------------------
if (idf:match("-100(%d+)") or is_owner(msg.sender_user_id_, msg.chat_id_)) and text == 'رفع المدراء'  then
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(MARKSTG..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
if not redis:get(MARKSTG.."dellmssg"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
if not redis:get(MARKSTG.."lock_pinn"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ التثبيت مقفول من قبل مدير عام عام 🍃", 1, "md")
return false
end
local id = msg.id_
local msgs = { [0] = id }
pinmsg(msg.chat_id_, msg.reply_to_message_id_, 0)
redis:set(MARKSTG.."pinnedmsg" .. msg.chat_id_, msg.reply_to_message_id_)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم  تثبيت الرسالۿ\n ✓ ", 1, 'md')
end end
----------info gp --------------------------------------------------
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and text:match('احصائيات المجموعه')  then
function gp_MARKSTG_info(arg,data)
local list = redis:smembers(MARKSTG.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(MARKSTG.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم مسح الرد بنجاح\n ✓ ", 1, 'md')
redis:del(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(MARKSTG..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(MARKSTG..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(MARKSTG..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(MARKSTG..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(MARKSTG..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(MARKSTG..'rep_owner'..msg.content_.text_..''..msg.chat_id_..'')
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد' and is_owner(msg.sender_user_id_, msg.chat_id_)  then
redis:set(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','set_repgp')
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل لي كلمه الرد الان ❗️\n',1, 'md')
return false    end
if msg.content_.text_ then
local content_keep = redis:get(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_keep == 'set_repgp' then
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n' ,  1, 'md')
redis:set(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','save_repgp')
redis:set(MARKSTG..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'',msg.content_.text_)
redis:sadd(MARKSTG..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
-------------------------------------------------------------------------
if  msg.content_.text_ == 'الردود' and is_owner(msg.sender_user_id_, msg.chat_id_) then
local redod = redis:smembers(MARKSTG..'rep_owner'..msg.chat_id_..'')
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(MARKSTG..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(MARKSTG..'gif_repgp'..v..msg.chat_id_)
redis:del(MARKSTG..'voice_repgp'..v..msg.chat_id_)
redis:del(MARKSTG..'stecker_repgp'..v..msg.chat_id_)
redis:del(MARKSTG..'video_repgp'..v..msg.chat_id_)
redis:del(MARKSTG..'text_repgp'..v..msg.chat_id_)
redis:del(MARKSTG..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم مسح جميع الردود\n ✓ ", 1, 'md')
return false
end
...
redis:set(MARKSTG.."add:repallt"..msg.sender_user_id_,'del_rep1')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد لمسحها ❗️\n",1, "md")
return false
end
if msg.content_.text_ then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم مسح الرد للكل\n ✓ ", 1, 'md')
redis:del(MARKSTG.."add:repallt"..msg.sender_user_id_)
redis:del(MARKSTG.."gif_repall"..msg.content_.text_)
redis:del(MARKSTG.."voice_repall"..msg.content_.text_)
redis:del(MARKSTG.."stecker_repall"..msg.content_.text_)
redis:del(MARKSTG.."video_repall"..msg.content_.text_)
redis:del(MARKSTG.."text_repall"..msg.content_.text_)
redis:del(MARKSTG.."rep_sudo",msg.content_.text_)
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد للكل' and is_sudo(msg)  then
redis:set(MARKSTG.."add:repallt"..msg.sender_user_id_,'set_rep')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد ❗️\n",1, "md")
return false    end
if msg.content_.text_ then
local content_keep = redis:get(MARKSTG.."add:repallt"..msg.sender_user_id_)
if content_keep == 'set_rep' then
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n" ,  1, "md")
redis:set(MARKSTG.."add:repallt"..msg.sender_user_id_,'save_rep')
redis:set(MARKSTG.."addreply2:"..msg.sender_user_id_, msg.content_.text_)
redis:sadd(MARKSTG.."rep_sudo",msg.content_.text_)
return false
end    end
------------------------------------------------------------------------------------
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) then
local redod = redis:smembers(MARKSTG.."rep_sudo")
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(MARKSTG.."add:repallt"..v)
redis:del(MARKSTG.."gif_repall"..v)
redis:del(MARKSTG.."voice_repall"..v)
redis:del(MARKSTG.."stecker_repall"..v)
redis:del(MARKSTG.."video_repall"..v)
redis:del(MARKSTG.."text_repall"..v)
redis:del(MARKSTG.."rep_sudo",msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmMARKSTG(msg).."\n🎟┊ تم مسح ردود المطور\n ✓ ", 1, 'md')
return false
...
local list = redis:smembers(MARKSTG.."bot:owners:" .. chattid)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(MARKSTG.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
...
local kpmonshis = redis:scard(MARKSTG.."bot:monshis:" .. chattid) or "0"
local kpbanned = redis:scard(MARKSTG.."bot:banned:" .. chattid) or "0"
local kpowners = redis:scard(MARKSTG.."bot:owners:" .. chattid) or "0"
local kpmuted = redis:scard(MARKSTG.."bot:muted:" .. chattid) or "0"
local kpkeed = redis:scard(MARKSTG.."bot:keed:" .. chattid) or "0"
local kpmomod = redis:scard(MARKSTG.."bot:momod:" .. chattid) or "0"
local kpvipmem = redis:scard(MARKSTG.."bot:vipmem:" .. chattid) or "0"
if not redis:get(MARKSTG.."bot:group:link"..chattid) then
local getlink = 'https://api.telegram.org/bot'..MARKSTG_TOKEN..'/exportChatInviteLink?chat_id='..chattid
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(MARKSTG.."bot:group:link"..chattid,link.result)
end
end
local lik_1 = redis:get(MARKSTG.."bot:group:link"..chattid)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(msg.chat_id_, msg.id_, 1, "🕴┊ مدير عام عام » ["..owner.."]\n🥈┊ﮧ ["..title_name(chattid).."]("..(link or "t.me/MARKSTG_ch")..")\n©️┊ عدد المدراء   » *"..kpowners.."*\n©️┊ عدد المنشئين   » *"..kpmonshis.."*\n🔆┊ عدد المدراء   » *"..kpmomod.."*\n🔰┊ عدد المكتومين  » *"..kpmuted.."*\n®️┊ عدد المحظورين » *"..kpbanned.."*\n®️┊ عدد المقيدين   » *"..kpkeed.."*\n🗯┊ عدد المميزين  » *"..kpvipmem.."*\n💠┊ﮧ `"..chattid.."`\n🏮┊ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
getChannelFull(chattid, gp_MARKSTG_info, nil)
end 
end
------------UNPIN-----------------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(MARKSTG_SUDO, 0, 0, 1, nil, './MARKSTG.lua', dl_cb, nil)
...
local pin_id = redis:get(MARKSTG.."pinnedmsg" .. msg.chat_id_)
if pin_id then
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم √ اعاده تثبيت الرسالۿ 🎐", 1, "md")
end
pinmsg(msg.chat_id_, pin_id, 0)
elseif redis:get(MARKSTG.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(MARKSTG.."lock_kickme"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ امر اطردني معطل 🍃", 1, "md")
return false
end
redis:set(MARKSTG.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:set(MARKSTG.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ ارسل *{ نعم }* لطردك\n🚫┊ ارسل *{ لا } *لالغاء طردك", 1, "md")
end
local Kpkick = redis:get(MARKSTG.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if Kpkick == "kick_mee" then
if text:match("^نعم$") then
if is_vipmem(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🌀┊ عذرا لا استطيع (حظر،طرد،كتم)المدراء والمدراء والاعضاء المميزين ❗️', 1, 'md')
else
local Kpkick = redis:get(MARKSTG.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if Kpkick == "kick_mee" then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del(MARKSTG.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(MARKSTG.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ تم طردتك حياتي", 1, "md")
end 
end
end
if text:match("^لا$") then
local Kpunkick = redis:get(MARKSTG.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
redis:set(MARKSTG.."bot:viewget" .. msg.sender_user_id_, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊ ارسل لي توجيۿ  للمنشور 🎈: ", 1, "md")
end end
-----------------SEE ACAUNT---------------------------------------
...
redis:set(MARKSTG.."addstart1"..msg.sender_user_id_, "theaddstarts")
return false
end end
if text then
local theaddstart = redis:get(MARKSTG.."addstart1"..msg.sender_user_id_)
if theaddstart == 'theaddstarts' then
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم حفظ الكليشه 🍃", 1, 'md')
redis:del(MARKSTG.."addstart1"..msg.sender_user_id_)
redis:set(MARKSTG.."startbot", text)
return false
end end
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
local msgs = (tonumber(redis:get(MARKSTG.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_)) or "0" )
local kptext = (tonumber(redis:get(MARKSTG.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpsticker = (tonumber(redis:get(MARKSTG.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pkPhoto = (tonumber(redis:get(MARKSTG.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVoice = (tonumber(redis:get(MARKSTG.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpGif = (tonumber(redis:get(MARKSTG.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVideo = (tonumber(redis:get(MARKSTG.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpSelfVideo = (tonumber(redis:get(MARKSTG.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpcon = (tonumber(redis:get(MARKSTG.."kpaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
--------------------------------------------------------
if text == ''..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' هينه' or text == ''..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' رزله' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(MARKSTG_SUDO) then
send(msg.chat_id_, msg.id_, 1, 'انجب لك هذا مطوري العشق 😌💋', 1, 'md')
return false  end
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, 'لك مگدر اهين نفسي 😞😂', 1, 'md')
return false  end
local MARKSTG = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐🍃","ها ابن الحنينه، ليش متسكت وتنجب 🌚"}
send(msg.chat_id_, result.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' بوسه' or text == ''..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(MARKSTG_SUDO) then
...
return false  end
local MARKSTG = { "اععع 🤢خده بي حب شباب الوصخ😹😹","موااح 💋 مواااح  حياتي💋😌🍃","💋😞نسخ لصق ع الشفه 👄"}
send(msg.chat_id_, result.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
--------------------------------- rdood bot --------------------------------------------------
if not redis:get(MARKSTG.."lock_reeeep"..msg.chat_id_) then
if text == "السلام عليكم" or text == "سلام عليكم" or text == "سلام" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {'اسمي '..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' 😌👌','تره اسمي '..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..'' then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {'اي نعم اني '..(redis:get(MARKSTG..'MARKSTGnams') or 'كيبر')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-------------------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"هلووووات  ⁽🙆♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"عٰٰٰود لوتُٰي 🙀 يٰگول بُِوت عبالٰه طافُٰٰي💔 ويضِٰل يمٰٓسلت وينشٰٰر روابٰٓط 😳🍃","اي بوت شتريد 😤" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "بوسني" or text == "بوسه" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then local MARKSTG = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"عيب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"كافي ضحك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"لصير عصبي يرتفع ضغطك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💋" or text == "💋💋" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"كبر 🙀 جان استحيتو 😹😹","عسل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💔" or text == "💔💔" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😐" or text == "🙂" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
---------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
------------------------------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(MARKSTG..'lock:add'..msg.chat_id_) then
local MARKSTG = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..MARKSTG[math.random(#MARKSTG)]..'', 1, 'md')
end
end
end
...
if idf:match("-100(%d+)") and not redis:get(MARKSTG.."bot:muted:Time" .. chat .. ":" .. user) and redis:sismember(MARKSTG.."bot:muted:" .. chat, user) then
redis:srem(MARKSTG.."bot:muted:" .. chat, user)
end
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(MARKSTG.."bot:links:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.entities_ and result.content_.entities_[0] and (result.content_.entities_[0].ID == "MessageEntityTextUrl" or result.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(MARKSTG.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.web_page_ and redis:get(MARKSTG.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if (text:match("[Hh][Tt][Tt][Pp]") or text:match("[Ww][Ww][Ww]") or text:match(".[Cc][Oo]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Rr]")) and redis:get(MARKSTG.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("@") and redis:get(MARKSTG.."tags:lock" .. msg.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("#") and redis:get(MARKSTG.."bot:hashtag:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("[\216-\219][\128-\191]") and redis:get(MARKSTG.."bot:arabic:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
...
if redis:get(MARKSTG..'bot:english:mute'..result.chat_id_) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end
end
if redis:get(MARKSTG..'editmsg'..msg.chat_id_) == 'delmsg' then
local id = msg.message_id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
elseif redis:get(MARKSTG..'editmsg'..msg.chat_id_) == 'didam' then
if redis:get(MARKSTG..'bot:editid'..msg.message_id_) then
local old_text = redis:get(MARKSTG..'bot:editid'..msg.message_id_)
...
redis:sadd(MARKSTG.."groups:users" .. msg.chat_id_, msg.sender_user_id_)
if text then
...
redis:set(MARKSTG.."Bot:KpBotAccount", data.value_.value_)
end end end end  end
----- End Source By marks MARKSTG »»»»»»»»»»»»»»»»»»»
