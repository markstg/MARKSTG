...
if text:match("^[/!#]([Ww][Ee][Aa][Tt][Hh][Ee][Rr]) (.*)$") or text:match("^(طقس) (.*)$") then 
    ...
    local F1,C1 = MARKSTG(jtab.main.temp)
    local F2,C2 = MARKSTG(jtab.main.temp_min)
    local F3,C3 = MARKSTG(jtab.main.temp_max)
    if jtab.rain then
    ...
    local jtab = json:decode(res)
    for i=1,5 do
    local F1,C1 = MARKSTG(jtab.list[i].main.temp_min)
    local F2,C2 = MARKSTG(jtab.list[i].main.temp_max)
    ...
    MARKSTG(msg.chat_id_, msg.id_, 1, Text, 1, 'md')
    else
